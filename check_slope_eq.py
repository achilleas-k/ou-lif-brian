...
Append data or quit [a/Q]: " % os.path.abspath(data_basepath))
    ...
    print "mo, ma, so, sa, fr, mslope, misi"
    for result in data.itervalues():
        mo = result.get('mu_offs')*1000
        ma = result.get('mu_amp')*1000
        so = result.get('sigma_offs')*1000
        sa = result.get('sigma_amp')*1000
        fr = result.get('freq')
        #tau = result.get('tau')*1000
        #v_th = result.get('v_th')*1000
        mslope = result.get('mslope')*1000
        ...
        classic OU LIF
        '''
        print '%f, '*7 % (mo, ma, so, sa, fr, mslope, misi)
